...
@section('content')
    ...
    <div style="background: #fff; border-radius: 16px; padding: 2rem; box-shadow: 0 4px 6px rgba(236, 72, 153, 0.15); max-width: 800px;">
        <div style="display: flex; align-items: center; gap: 1.5rem; margin-bottom: 2rem;">
            ...
            <div>
                <h2 style="margin: 0; color: #4b1f3a;">{{ $profile->user->name ?? 'No User' }}</h2>
        ...
        <div class="action-group" style="justify-content: flex-start; margin-top: 2rem;">
            <a href="{{ route('profiles.edit', $profile->id) }}" class="action-btn action-btn-edit">Edit</a>
            <a href="{{ route('profiles.index') }}" class="action-btn action-btn-back">Back</a>
        </div>
    </div>
@endsection
