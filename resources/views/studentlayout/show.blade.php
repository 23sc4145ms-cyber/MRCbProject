    ...
    <div style="display: grid; grid-template-columns: 1fr; gap: 2rem;">
        ...
        <div style="background: #fff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px rgba(236, 72, 153, 0.15);">
            ...
            <div style="padding: 2rem;">
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 2rem;">
                    ...
                    <div style="padding: 1.5rem; background: linear-gradient(135deg, #dbeafe 0%, #e0f2fe 100%); border-radius: 12px; border-left: 4px solid #06b6d4;">
                        <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.75rem;">
                            ...
                            <label style="color: #06b6d4; font-weight: 600; font-size: 0.875rem; text-transform: uppercase; letter-spacing: 0.5px; margin: 0;">Degree</label>
                        </div>
                        <p style="margin: 0; color: #374151; font-size: 1.125rem; font-weight: 600;">{{ $student->degree->name ?? $student->degree->Degree ?? 'Not Assigned' }}</p>
                    </div>
                ...
                <div class="action-group" style="justify-content: flex-start; margin-top: 2rem; padding-top: 2rem; border-top: 1px solid #fce7f3;">
                    <a href="/students/{{ $student->id }}/edit" class="action-btn action-btn-edit">Edit Information</a>
                    <form action="/students/{{ $student->id }}" method="POST" style="display: inline;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="action-btn action-btn-delete">Delete Record</button>
                    </form>
                    <a href="{{ route('students.index') }}" class="action-btn action-btn-back">Back to Student List</a>
                </div>
            </div>
        </div>
        @endforeach
